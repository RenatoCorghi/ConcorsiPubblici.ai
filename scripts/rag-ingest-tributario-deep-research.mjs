...
const BASE_DIR = path.join(process.cwd(), 'schede_tributario_vip', 'deep_research');

async function ingestTributarioDeepResearch() {
    console.log("🚀 AVVIO INGESTIONE VIP TRIBUTARIO (Deep Research)");
    ...
    for (const file of files) {
        ...
        try {
            ...
            const docUuid = generateUUID(`trib_vip_${file}`);
            
            // Estrazione metadati dal contenuto della scheda
            let titolo = file.replace('.md', '');
            let materia = 'Diritto Tributario';
            let autorita = 'Open Access VIP';
...
ingestTributarioDeepResearch().catch(console.error);
