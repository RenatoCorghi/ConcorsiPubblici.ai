...
const INPUT_DIR = path.join(process.cwd(), 'data', 'Deep Research tributario');
const OUTPUT_DIR = path.join(process.cwd(), 'schede_tributario_vip', 'deep_research');

fs.mkdirSync(OUTPUT_DIR, { recursive: true });

const SYSTEM_PROMPT = `Sei un Magistrato, Professore Universitario e Senior Data Engineer.
Il tuo compito è analizzare complessi testi di approfondimento dottrinale (Deep Research) in materia di Diritto Tributario e Processuale Tributario (es. Abuso del diritto, Accertamento, Riscossione, Processo).

[REGOLE DI ESTRAZIONE]
1. Il testo contiene trattazioni approfondite. Il tuo compito è individuare i "Macro-Argomenti" (istituti giuridici) e creare una Scheda VIP per ciascuno.
...
- "titolo_file": Un nome file logico (es. "dottrina_abuso_diritto", "dottrina_litisconsorzio_tributario")
- "materia": Usa sempre "Diritto Tributario"
- "rilevanza_concorsuale": "Alta", "Media" o "Bassa"
- "tags": es. "#Accertamento #Elusione #Processo"
- "autorita": Usa "Approfondimento Dottrinale / Deep Research"
- "provvedimento": Usa il titolo o la macro-area (es. "Analisi Dottrinale")
- "thema_decidendum": Inquadramento Sistematico dell'istituto (Di cosa stiamo parlando).
- "quadro_normativo": Le leggi e i riferimenti normativi (es. Statuto Contribuente, TUIR, d.lgs. 546/92).
...
- "spendibilita": Consigli concorsuali (es. "Ottimo per temi sui poteri dell'Agenzia delle Entrate").
...
async function main() {
    console.log(`💎 Generazione Schede VIP Dottrina Tributario\n`);
