/**
 * COLLETTORE ULTIMATE ID TRIBUTARIE 2025
 *
 * Unisce tutte le migliori strategie confermate:
 * - Puppeteer headless:false (bypassa Akamai)
 * - Warmup umano iniziale (10s)
 * - Input dei campi React via eventi e coordinazione
 * - Paginazione robusta (fino a 80 pagine)
 * - Salvataggio checkpoint continui (ogni 5 pagine) in scratch/tributaria_ids_2025.json
 */
...
const OUT_FILE = path.join(SCRATCH_DIR, 'tributaria_ids_2024_test.json');
...
async function main() {
    log('🚀 Avvio Collettore Ultimate ID 2025...');
    ...
    try {
        ...
        // Aspetta i risultati (con gestione dell'errore server 500)
        ...
        try {
            ...
            await page.screenshot({ path: path.join(SCRATCH_DIR, 'ultimate_timeout.png') });
            await browser.close();
            process.exit(1);
        }
