-- Migration 007: Fix function overloading conflict (PGRST203)
-- Problema: PostgREST vede multiple versioni di match_documents_hybrid
-- con parametri leggermente diversi (float vs double precision) e non sa
-- quale usare → ritorna 300 con "Could not choose the best candidate function"
-- Soluzione: droppare TUTTE le versioni e ricreare solo la v6

-- DROP tutte le possibili firme
DROP FUNCTION IF EXISTS public.match_documents_hybrid(vector, text, int, float, text, text, int, int);
DROP FUNCTION IF EXISTS public.match_documents_hybrid(vector, text, int, double precision, text, text, int, int);
DROP FUNCTION IF EXISTS public.match_documents_hybrid(vector(768), text, int, float, text, text, int, int);
DROP FUNCTION IF EXISTS public.match_documents_hybrid(vector(768), text, int, double precision, text, text, int, int);
DROP FUNCTION IF EXISTS public.match_documents_hybrid(extensions.vector, text, int, float, text, text, int, int);
DROP FUNCTION IF EXISTS public.match_documents_hybrid(extensions.vector, text, int, double precision, text, text, int, int);
DROP FUNCTION IF EXISTS public.match_documents_hybrid(extensions.vector(768), text, int, float, text, text, int, int);
DROP FUNCTION IF EXISTS public.match_documents_hybrid(extensions.vector(768), text, int, double precision, text, text, int, int);
