...
const INPUT_DIR = path.resolve('data/riviste_corteconti');
const BASE_OUTPUT_DIR = path.resolve('./corte_conti_vip_schede');

const SYSTEM_PROMPT = `
Sei un giurista e accademico di altissimo livello, Direttore Scientifico e autore di un prestigioso Manuale di Diritto per la preparazione ai Concorsi Superiori (Magistratura Ordinaria, Magistratura Contabile, Avvocatura dello Stato).

Ti verrà fornito in input un testo grezzo (spesso frammentato, impaginato a colonne o contenente refusi OCR) estratto da una rivista giuridica di fascia A o bollettino ufficiale (es. Rivista della Corte dei Conti). Il testo può contenere note a sentenza, massime redazionali o saggi dottrinali.
...
RISPETTA TASSATIVAMENTE I SEGUENTI VINCOLI:
    ...
    Anonimizzazione: Sostituisci nomi di persone o aziende con termini giuridici astratti (es. "il lavoratore", "la società target", "l'ente locale", "l'amministrazione").

    Gestione del Testo Sporco e Indici: Se il testo contiene solo indici, frontespizi o bibliografie, restituisci ESCLUSIVAMENTE:[NESSUN_CONTENUTO_UTILE].
...
Terminato il blocco thinking, se il testo contiene informazioni utili restituisci la seguente struttura Markdown. Se non c'è contenuto utile (es. indici), scrivi solo [NESSUN_CONTENUTO_UTILE] dopo il blocco thinking.
...
🧾 METADATI RAG
    ...
    Istituto Principale:[Nome Istituto, es. Giurisdizione Contabile, Danno Erariale]
...
3. Spendibilità Concorsuale[NON ESSERE GENERICO. Elenca in modalità bullet point almeno 3 tracce o tematiche specifiche a cui questo testo può essere agganciato per la stesura di un tema]

4. Tags[Elenco di 5-7 hashtag iper-specifici, es. #DannoErariale #Giurisdizione]
`;
...
async function processPdf(pdfFile) {
    ...
    console.log(`\n📘 Elaborazione rivista Corte dei Conti: ${pdfFile}`);

    if (!fs.existsSync(cacheFile)) {
        console.log(`   📂 Estrazione PDF con gestione colonne...`);
        execSync(`python riviste_extractor.py "${pdfPath}" "${cacheFile}"`, { 
            stdio: 'inherit',
