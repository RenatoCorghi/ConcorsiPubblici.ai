...
const BASE_DIR = path.join(process.cwd(), 'schede_civile_vip', 'notariato');

async function ingestNotariatoVIP() {
    console.log("🚀 AVVIO INGESTIONE VIP NOTARIATO (Studi CNN)");
    ...
    for (const file of files) {
        ...
        try {
            ...
            const docUuid = generateUUID(`nota_vip_${file}`);
            ...
            let materia = 'Diritto Civile (Notariato)';
            let autorita = 'Consiglio Nazionale del Notariato';
...
ingestNotariatoVIP().catch(console.error);
