...
const DOC_PATH = path.join(process.cwd(), 'data', 'Deep Research tributario', 'Ricerca Fonti Open Access Diritto Penale.docx');
const OUTPUT_DIR = path.join(process.cwd(), 'schede_penale_vip', 'dottrina_oa');

fs.mkdirSync(OUTPUT_DIR, { recursive: true });

const SYSTEM_PROMPT = `Sei un esperto accademico e Magistrato.
Il tuo compito è analizzare trattati dottrinali e massime di Diritto Penale in Open Access.
...
- "titolo_file": Un nome file logico (es. "dottrina_concorso_persone")
- "materia": Usa "Diritto Penale"
- "rilevanza_concorsuale": "Alta", "Media" o "Bassa"
- "tags": es. "#Concorso #Reato"
...
async function main() {
    console.log(`💎 Generazione Schede VIP Penale (Docx Deep Research)\n`);
